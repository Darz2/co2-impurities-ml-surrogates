...
AL_TYPE="${1:?Usage: sbatch $0 AL_TYPE SIZE  (e.g. AL_ST N025)}"
SIZE="${2:?Usage: sbatch $0 AL_TYPE SIZE  (e.g. AL_ST N025)}"

echo "Job started at: $(date '+%F %T')"
echo "AL_TYPE: ${AL_TYPE}  Size: ${SIZE}"
...
COMPOSITIONS_CSV="${AL_TYPE}/AL/AL_${SIZE}.csv"
...
if [ -z "${SLURM_ARRAY_TASK_ID:-}" ]; then
    ...
    sbatch --array="0-${LAST_INDEX}" "$0" "${AL_TYPE}" "${SIZE}"
    exit 0
fi
...
OUTPUT_DIR="RESULTS/${AL_TYPE}/${SIZE}/${ARRAY_JOB_ID}_${TASK_ID}"
