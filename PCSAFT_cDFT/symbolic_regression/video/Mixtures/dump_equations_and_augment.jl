# dump_equations_and_augment.jl
# (a) Dump the full Pareto hall of fame to a readable CSV/TXT.
# (b) Rewrite the SR prediction CSVs with the input feature columns added,
#     so the equation is verifiable per-row.
#
# Run from this directory:   julia dump_equations_and_augment.jl
...
# Must match the operators used during training in Mixture_V2.jl
...
target   = :eps_base
features = [:z_total_impurity, :x_total_impurity, :y_total_impurity,
...
# IDENTICAL split (same seed as Mixture_V2.jl)
n   = size(X, 1)
...
# Chosen equation = index 3 (manual_best_idx in Mixture_V2.jl)
best_idx   = 3
best_eq    = dominating[best_idx]
println("\nUsing chosen equation index = $best_idx : ", string_tree(best_eq.tree, options))
