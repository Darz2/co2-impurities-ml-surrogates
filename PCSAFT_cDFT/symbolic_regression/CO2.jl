...
annotate!(p_gamma, xmin + 0.52*(xmax - xmin), xmax - 0.78*(xmax - xmin),
    text("Train: RMSE = $(round(rmse_train, sigdigits=4)), R² = $(round(r2_train, sigdigits=4))", 9, :left))
annotate!(p_gamma, xmin + 0.52*(xmax - xmin), xmax - 0.84*(xmax - xmin),
    text("Val: RMSE = $(round(rmse_val, sigdigits=4)), R² = $(round(r2_val, sigdigits=4))", 9, :left))
annotate!(p_gamma, xmin + 0.52*(xmax - xmin), xmax - 0.90*(xmax - xmin),
    text("Test: RMSE = $(round(rmse_test, sigdigits=4)), R² = $(round(r2_test, sigdigits=4))", 9, :left))
